...
class MakefileTest < Minitest::Test
  def setup
    @atreus = Makefile.new 'test/fixtures/atreus/rules.mk'
  end

  def test_basic_value
    value = @atreus.get :mcu
    assert_equal value, 'atmega32u4'
  end

  def test_complex_value
    value = @atreus.get :opt_defs
    assert_equal value, '-DATREUS_ASTAR -DINTERRUPT_CONTROL_ENDPOINT'
  end

  def test_logical_value
    planck = Makefile.new 'test/fixtures/planck/rules.mk'

    value = planck.get :bootloader
    assert_nil value
  end
end
